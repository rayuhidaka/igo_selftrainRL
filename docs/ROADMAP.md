...
- [x] **Generation 11, past the original gen1-10 target, resolved on
      retry (2026-09-09/10):** also wired real GPU fine-tuning (RTX 3050
      via WSL, `bootstrap.train`'s new `device: auto/cpu/cuda` config
      key) for the training step only — self-play and eval stay
      ...
      plateau. **Attempt 2** (fresh seed 42424242, same recipe): PROMOTE
      against both gen10 (1578.0 vs. 1422.0, a 156-Elo gap — the most
      decisive gen-to-gen win since generation 9's 223-Elo jump) and the
      pristine baseline (1653.2 vs. 1346.8). `bootstrap_gen11_no_pass_guard_candidate.pt`
      (attempt 2) is the new best checkpoint — eleven chained generations
      overall, the first past the original gen1-10 target. Both attempts
      ran unattended overnight in a detached WSL `tmux` session
      (`run_gen11.sh`, self-play → GPU fine-tune → eval x2), surviving a
      machine reboot between them since the run itself had already
      finished writing its log before the reboot happened. Not yet
      exported to `.tflite` or wired into igo-app's difficulty picker —
      that's a separate decision, see igo-app's own `docs/ROADMAP.md`.
...
- [ ] **Blocking prerequisite for any further generation (gen12+),
      ...
      **A second, concrete cause implemented and validated (2026-09-10):**
      this pipeline never applied the board's 8-fold dihedral symmetry to
      training data (standard AlphaZero/AlphaGo Zero practice) — grepped
      `bootstrap/`/`selfplay/` for rotate/flip/symmetry/augment, nothing
      real. New `bootstrap/dataset.py`: `SelfPlayDataset`/
      `build_training_loader` gained an `augment` parameter (default
      `False`, every existing config unaffected), threaded through
      `bootstrap/train.py`'s new `config["augment"]` key —
      `_apply_dihedral_transform` applies one of the 8 board symmetries
      to `board_planes` and `policy_targets` *consistently* (the critical
      property; tested in `tests/test_dataset.py`, 6 new tests, full
      suite 88/88 passing). Validated with
      `configs/bootstrap_train_gen11_augment_candidate.yaml` — gen11's
      exact recipe (same warm start, data, seed, 60s budget), only
      `augment: true` added, no new self-play needed. **Result: the
      augmented checkpoint's top-5 empty-board moves became all 4 board
      corners at roughly equal probability (~3-4% each) — confirms the
      diagnosis (the net had learned one specific corner, not "corners in
      general") — versus the unaugmented gen11's skewed cluster on one
      edge ((8,8) 13.7%, (8,7) 13.1%).** Not a full fix yet — corners are
      still preferred over star points even augmented, expected from one
      short nudge on top of self-play data generated by pre-augmentation
      search. **Next, not yet done:** a real (not 60s-throwaway) training
      run with augmentation, likely needed from earlier in the chain
      (not just one late fine-tune) to see whether star-point opens
      actually emerge, before this item can be marked resolved.

      **Follow-up plan scoped and pre-staged (2026-09-10), training to
      start 2026-09-11:**
      1. **Option 1 — `./run_option1_augment_extended.sh`.** Continues
         training from `bootstrap_gen11_augment_candidate.pt` (already
         confirmed: restores rotational symmetry) for 900s (vs. the
         standard 60s), same data, no new self-play. Also runs
         `tools.spotcheck_opening` and a strength check (`eval.promote`
         vs. plain gen11 and vs. the pristine baseline) — a training-only
         change needs the same promotion gate as any other candidate, not
         just an opening-move look. Cheap (~15min fine-tune + ~15-20min
         eval); doesn't need a detached tmux session.
      2. **Decide, from Option 1's spot-check + eval output:** if it
         promotes against plain gen11 *and* shows real movement toward
         star points (not just restored corner symmetry), use it as
         generation 12's self-play generator and chain parent — update
         `checkpoint_path` in `configs/selfplay_self_play_gen12_augment.yaml`
         and `init_from_checkpoint` in
         `configs/bootstrap_train_gen12_augment_candidate.yaml` from plain
         gen11 to `bootstrap_gen11_augment_extended_candidate.pt` first.
         Otherwise leave both at their default (plain gen11) and treat
         Option 1 as informative-but-insufficient.
      3. **Option 2 — `./run_option2_gen12_augment.sh`.** A real
         generation-12 cycle (800-game self-play, standard 60s
         `augment: true` fine-tune, spot-check, eval vs. gen11 and vs.
         baseline) — same recipe as every prior generation, only
         augmentation added. **Run self-play in your own detached WSL
         `tmux` session** (this project's established practice — see
         `run_gen11.sh`'s own note and
         [[feedback_long_background_jobs_this_machine]]), hours not
         minutes.
      4. **If neither closes the gap** (star points still not
         competitive): fall back to Option 3 from the prior scoping
         conversation — restart from the pristine baseline with
