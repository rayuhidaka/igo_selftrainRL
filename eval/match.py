...
def play_match(
    ...
    plays Black each game. `channels_*`/`num_conv_layers_*` are only needed for a *legacy*
    checkpoint (saved before bootstrap/checkpoint.py existed, so its architecture isn't
    recorded) or to deliberately override -- see `RayZeroPolicyValueNet`. They need not
    match each other, e.g. comparing checkpoints from before/after an architecture change.
    """
    net_a = RayZeroPolicyValueNet(checkpoint_a, board_size, channels=channels_a, num_conv_layers=num_conv_layers_a)
    net_b = RayZeroPolicyValueNet(checkpoint_b, board_size, channels=channels_b, num_conv_layers=num_conv_layers_b)
